...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        int estimatedIOCost = minOp.estimateIOCost();
        List<Integer> indexColumns = getEligibleIndexColumns(table);
        int index = -1;

        for (int i : indexColumns) {
            SelectPredicate sp = selectPredicates.get(i);
            QueryOperator maybe = new IndexScanOperator(transaction, table, sp.column, sp.operator, sp.value);
            if (maybe.estimateIOCost() < estimatedIOCost) {
                minOp = maybe;
                estimatedIOCost = maybe.estimateIOCost();
                index = i;
            }
        }
     ...
     *                 table access (scan) query operator. "1 table --> 1 QO"
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        for (Set<String> tables : prevMap.keySet()) {
            for (JoinPredicate jp : this.joinPredicates) {
                ...
                Set<String> idToFetch = new HashSet<>();
                QueryOperator fetched, cheapest;
                Set<String> resultId = new HashSet<>(tables);
                if (tables.contains(lT) && !tables.contains(rT)) {
                    idToFetch.add(rT);
                    fetched = pass1Map.get(idToFetch);
                    cheapest = minCostJoinType(prevMap.get(tables), fetched, lC, rC);
                    resultId.add(rT);
                } else if (tables.contains(rT) && !tables.contains(lT)) {
                    idToFetch.add(lT);
                    fetched = pass1Map.get(idToFetch);
                    cheapest = minCostJoinType(prevMap.get(tables), fetched, rC, lC);
                    resultId.add(lT);
                } else {
                    continue;
                }
                ...
                if (result.containsKey(resultId)) {
                    if (result.get(resultId).estimateIOCost() > cheapest.estimateIOCost()) {
                        result.put(resultId, cheapest);
                    }
                } else {
                    result.put(resultId, cheapest);
                }
        ...
        return result; // all table in 1 set --> 1 QO
    }

    // Task 7: Optimal Plan Selection //////////////////////////////////////////

    /**
     * Finds the lowest cost QueryOperator in the given mapping.
     * A mapping is generated on each pass of the search algorithm,
     * and relates a set of tables to the lowest cost QueryOperator accessing those tables.
     *
    ...
    public Iterator<Record> execute() {
        ...
        // pass 1
        Map<Set<String>, QueryOperator> tableToLowestOperator = new HashMap<>();
        for (String tableName : this.tableNames) {
            Set<String> tableNameId = new HashSet<>();
            tableNameId.add(tableName);
            QueryOperator lowestOperator = minCostSingleAccess(tableName);
            tableToLowestOperator.put(tableNameId, lowestOperator);
        }

        Map<Set<String>, QueryOperator> pass1Map = new HashMap<>(tableToLowestOperator);
        // pass i
        while (tableToLowestOperator.size() > 1) {
            // Repeat until all tables have been joined!
            tableToLowestOperator = minCostJoins(tableToLowestOperator, pass1Map);
        }
        this.finalOperator = tableToLowestOperator.get(tableToLowestOperator.keySet().iterator().next());
