...
class LeafNode extends BPlusNode {
    ...
    LeafNode(BPlusTreeMetadata metadata, BufferManager bufferManager,
             List<DataBox> keys, List<RecordId> rids, Optional<Long> rightSibling,
    ...
    public LeafNode get(DataBox key) {
        ...
        // only care about inner node? WRONG?
        return this;
    }
    ...
    public LeafNode getLeftmostLeaf() {
        // TODO(proj2): implement
        // if is leaf, then return the leaf...
        return this;
    }
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        if (this.keys.contains(key)) {
            throw new BPlusTreeException("Key " + key + " is already used!");
        }
        // put all in TreeMap
        TreeMap<DataBox, RecordId> tmp = new TreeMap<>();
        // 这里是一一对应的, 使用map细节不多
        for (int i = 0; i < this.keys.size(); i++) {
            tmp.put(this.keys.get(i), this.rids.get(i));
        }
        tmp.put(key, rid); // possible max size = 2d+1
        // if overflow......
        int d = this.metadata.getOrder();
        if (tmp.keySet().size() == 2*d+1) {
            List<DataBox> addedKeys = new ArrayList<>(tmp.keySet());
            List<RecordId> addedRids = new ArrayList<>(tmp.values());

            List<DataBox> leftKeys = new ArrayList<>(addedKeys.subList(0, d));
            List<RecordId> leftRids = new ArrayList<>(addedRids.subList(0, d));
            List<DataBox> rightKeys = new ArrayList<>(addedKeys.subList(d, 2*d+1));
            List<RecordId> rightRids = new ArrayList<>(addedRids.subList(d, 2*d+1)); // [a, b)

            // create and reset the RS
            // fetch new page!
            LeafNode newLeafNode = new LeafNode(
                    this.metadata, this.bufferManager,
                    rightKeys, rightRids, this.rightSibling, treeContext
            );
            DataBox splitKey = rightKeys.get(0);
            long newPageId = newLeafNode.page.getPageNum();
            // stayed in original page, THIS can not be redistributed!
            this.keys = leftKeys;
            this.rids = leftRids;
            this.rightSibling = Optional.of(newPageId);
            // return the pair (split_key, right_node_page_num)
            newLeafNode.sync();
            this.sync();
            return Optional.of(new Pair<>(splitKey, newPageId));
        }

        // no overflow...
        this.keys = new ArrayList<>(tmp.keySet());
        this.rids = new ArrayList<>(tmp.values());
        this.sync();
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        if (fillFactor<=0 || fillFactor>1) {
            throw new BPlusTreeException("Fill factor must fall into (0, 1]!");
        }
        int d = this.metadata.getOrder();
        int b = (int) Math.ceil(2*d*fillFactor); // round up

        while (this.keys.size() < b && data.hasNext()) {
        ...
        Pair<DataBox, RecordId> splitPair = data.next();
        newKeys.add(splitPair.getFirst());
        newRids.add(splitPair.getSecond());


        LeafNode newRight = new LeafNode(
          this.metadata, this.bufferManager, newKeys, newRids, Optional.empty(), treeContext
        );
        this.rightSibling = Optional.of(newRight.page.getPageNum());
        this.sync();
        newRight.sync();
        return Optional.of(new Pair<>(splitPair.getFirst(), newRight.page.getPageNum()));
    }
    ...
    public byte[] toBytes() {
        ...
        //     a               b                   c                         d
        //    type + ID of RS + number of pairs + list of pairs (key, rid)
...
//        System.out.println(metadata.getOrder()*2);
//        //
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        Buffer buf = page.getBuffer(); // type + ID of RS + number of pairs + list of pairs

        byte nodeType = buf.get(); // get the type
        assert(nodeType == (byte) 1);
        // get the id of RS
        Optional<Long> rs = null;
        ...
        return new LeafNode(metadata, bufferManager, page, keys, rids, rs, treeContext);
    }
