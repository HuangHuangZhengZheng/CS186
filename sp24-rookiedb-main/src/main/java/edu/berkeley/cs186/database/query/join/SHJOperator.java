...
public class SHJOperator extends JoinOperator {
    private int numBuffers; // B
    private Run joinedRecords; // a run is a sorted sequence of records? pages!
    ...
    public SHJOperator(QueryOperator leftSource,
        ...
        super(leftSource, rightSource, leftColumnName, rightColumnName, transaction, JoinType.SHJ); // **kwargs hhh
    ...
    public int estimateIOCost() {
        ...
        // maximum possible cost to encourage the optimizer to avoid it...... wow !
    ...
    public BacktrackingIterator<Record> backtrackingIterator() {
        ...
        return joinedRecords.iterator(); // run
    }
     ...
     *
     * i.e. we build partitionS here, same hash val in same p[i]
     */
     ...
     * joinedRecords list.
     *
     * @param partition a partition, a single partition!
    ...
    private void buildAndProbe(Partition partition, Iterable<Record> rightRecords) {
        ...
        for (Record leftRecord: partition) {
            DataBox leftJoinValue = leftRecord.getValue(this.getLeftColumnIndex()); // get the "value"
            if (!hashTable.containsKey(leftJoinValue)) {
                hashTable.put(leftJoinValue, new ArrayList<>());
            }
            hashTable.get(leftJoinValue).add(leftRecord); // hash table: leftJoinValue of record --> leftRecord(s)
        }
        ...
        for (Record rightRecord: rightRecords) {
            ...
            for (Record lRecord : hashTable.get(rightJoinValue)) {
                ...
                // Accumulate joined records in this.joinedRecords, i.e. add into the Run
    ...
    private void run(Iterable<Record> leftRecords, Iterable<Record> rightRecords, int pass) {
        ...
        // Partition records into left and right --> PartitionS
