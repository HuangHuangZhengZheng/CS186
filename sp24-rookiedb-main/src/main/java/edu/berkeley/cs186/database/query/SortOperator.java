...
public class SortOperator extends QueryOperator {
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        // 对于iterator的经验教训：减少多次访问or生成，尽可能拿出来静态使用
        List<BacktrackingIterator<Record>> iters = new ArrayList<>(runs.size());
        for (Run run : runs) {
            iters.add(run.iterator());
        }

        for (int i = 0; i < runs.size(); i++) {
            iterator = iters.get(i);
            if (iterator.hasNext()) {
        ...
        while (!pq.isEmpty()) {
            ...
            Record current = p.getFirst();
            ans.add(current);

            int currentIndex = p.getSecond();
            Iterator<Record> iter = iters.get(currentIndex);
            if (iter.hasNext()) {
                pq.add(new Pair<>(iter.next(), currentIndex));
            }
        }
        return ans;
    }
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        int batch = this.numBuffers - 1;
        for (int i = 0; i < runs.size(); i += batch) {
            int end = Math.min(runs.size(), i + batch);
            ans.add(mergeSortedRuns(runs.subList(i, end)));
        }
        return ans;
    }
    ...
    public Run sort() {
        ...
        while (sourceIterator.hasNext()) {
            Iterator<Record> b = getBlockIterator(sourceIterator, getSchema(), numBuffers);
            Run r = sortRun(b);
            runs.add(r);
        }
        // 经典merge操作
        ...
        ans = runs.get(0);
        return ans; // TODO(proj3_part1): replace this!
    }
