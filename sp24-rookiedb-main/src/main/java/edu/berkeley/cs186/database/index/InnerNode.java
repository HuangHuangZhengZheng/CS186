...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        ...
        // find the correct child's id
        long targetId = key.getInt();
        int i = 0;
        for (DataBox idBox : this.keys) {
            if (targetId >= idBox.getInt()) {
                i++;
            }
        }
        // 用基类来看待
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        // TODO(proj2): implement
        // if overflow
        long targetId = key.getDataBoxValueAsLong();

         // in proj3, cause a bug!
        int i = 0;
        for (DataBox idBox : this.keys) {
            long idBoxValue;

            if (targetId >= idBox.getDataBoxValueAsLong()) {
                i++;
            }
        }
        // recursion
        Optional<Pair<DataBox, Long>> pair = getChild(i).put(key, rid);

        if (pair.isPresent()) {
            // if k keys, k+1 ptr!
            TreeMap<DataBox, Long> tmp = new TreeMap<>();
            for (int j = 0; j < this.keys.size(); j++) {
                tmp.put(this.keys.get(j), this.children.get(j+1));
            }
            tmp.put(pair.get().getFirst(), pair.get().getSecond());

            int d = this.metadata.getOrder();
            // insert into inner node and if overflow
            if (tmp.keySet().size() == 2*d+1) {
                List<DataBox> addedKeys = new ArrayList<>(tmp.keySet());
                List<Long> addedChildren = new ArrayList<>();
                addedChildren.add(this.children.get(0));
                addedChildren.addAll(tmp.values());

                List<DataBox> leftKeys = new ArrayList<>(addedKeys.subList(0, d));
                List<Long> leftChildren = new ArrayList<>(addedChildren.subList(0, d+1));
                List<DataBox> rightKeys = new ArrayList<>(addedKeys.subList(d+1, 2*d+1));
                List<Long> rightChildren = new ArrayList<>(addedChildren.subList(d+1, 2*d+2));

                // stayed
                this.keys = leftKeys;
                this.children = leftChildren;

                InnerNode rightInnerNode = new InnerNode(
                        this.metadata, this.bufferManager, rightKeys, rightChildren, treeContext
                ); // new page

                DataBox splitKey = addedKeys.get(d);
                long rightPageId = rightInnerNode.getPage().getPageNum(); // 6

                this.sync();
                rightInnerNode.sync();
                return Optional.of(new Pair<>(splitKey, rightPageId));
            }
            // insert into inner node but no overflow
            this.keys = new ArrayList<>(tmp.keySet());

            List<Long> newChildren = new ArrayList<>();
            newChildren.add(this.children.get(0)); // ?
            newChildren.addAll(tmp.values());
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
            float fillFactor) {
        // TODO(proj2): implement
        Optional<Pair<DataBox, Long>> pair = getChild(this.keys.size()).bulkLoad(data, fillFactor);
        if (pair.isPresent()) {
            // add into...
            ...
            if (this.keys.size() == 2*d+1) {
                ...
                leftKeys.addAll(this.keys.subList(0, d));
                rightKeys.addAll(this.keys.subList(d+1, 2*d+1));
                DataBox splitKey = this.keys.get(d);

                leftChildren.addAll(this.children.subList(0, d+1));
                rightChildren.addAll(this.children.subList(d+1, 2*d+2));
                ...
                this.sync();
                newRightNode.sync();
                return Optional.of(new Pair<>(splitKey, newRightNode.getPage().getPageNum()));
            }
        }
        this.sync();
        return Optional.empty();
    ...
    public void remove(DataBox key) {
        // TODO(proj2): implement
        LeafNode possible = this.get(key);
        possible.remove(key);
        this.sync();
        return;
    }
    ...
    public byte[] toBytes() {
        ...
        //     a         b        c                           d
        //     type + numbers of keys + list of keys (n) + list of children (n+1)
    ...
    public static InnerNode fromBytes(BPlusTreeMetadata metadata,
        ...
        Buffer buf = page.getBuffer(); // type + numbers of keys + list of keys (n) + list of children (n+1)

        byte nodeType = buf.get(); // get the type
        assert(nodeType == (byte) 0);

        List<DataBox> keys = new ArrayList<>();
        List<Long> children = new ArrayList<>();
        int n = buf.getInt(); // get the number
