...
public class GHJOperator extends JoinOperator {
    ...
    public int estimateIOCost() {
        ...
        // maximum possible cost to encourage the optimizer to avoid it ahh????
    ...
    private void buildAndProbe(Partition leftPartition, Partition rightPartition) {
        ...
        for (Record record : probeRecords) {
            ...
            for (Record l : hashTable.get(joinValue)) {
                // NOTE: L + R != R + L
                Record joinedRecord = probeFirst ? record.concat(l) : l.concat(record);
    ...
    private void run(Iterable<Record> leftRecords, Iterable<Record> rightRecords, int pass) {
        ...
        // Partition records into left and right, note that they have same length!
    ...
    public static Pair<List<Record>, List<Record>> getBreakSHJInputs() {
        ...
        // SHJ breaks when trying to join them but not GHJ  i.e. generate corner cases
